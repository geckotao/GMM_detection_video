...
def resource_path(filename: str) -> str:
    ...
    if "__compiled__" in globals():
        ...
        # Python 3.8+ 支持 add_dll_directory
...
def get_pictures_dir() -> str:
    """获取当前用户的"图片"文件夹（Win7 ~ Win11 通用）"""
    if os.name == 'nt':
        try:
            buf = ctypes.create_unicode_buffer(260)
            # CSIDL_MYPICTURES = 0x0027
            hr = ctypes.windll.shell32.SHGetFolderPathW(None, 0x0027, None, 0, buf)
...
def setup_dpi_awareness():
    ...
    major = get_windows_major_version()
...
class VideoProcessor:
    def __init__(self, gmm_var: int, fd_var: int, roi_mask: Optional[np.ndarray] = None):
        self.gmm_var = int(gmm_var)
        self.fd_var = int(fd_var)
    ...
    def preprocess_frame(self, frame: np.ndarray, target_height: int) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if target_height > 0 and frame.shape[0] > target_height:
        ...
        new_kernel_size = max(3, min(9, frame.shape[1] // 100))
        if new_kernel_size != self.kernel_size or self.kernel is None:
        ...
        if self.roi_mask is not None:
            # 防止 ROI mask 与当前帧尺寸不一致导致 OpenCV 报错
            if self.roi_mask.shape[:2] != gray.shape[:2]:
    ...
    def detect_change(self, gray: np.ndarray) -> Tuple[bool, np.ndarray, float]:
        ...
        if self.prev_gray is None:
            ...
            for _ in range(GMM_PREHEAT_FRAMES):
                self.gmm.apply(gray_blur)
            return False, np.zeros_like(gray), 0.0

        prev_gray_blur = cv2.GaussianBlur(self.prev_gray, (3, 3), 0)

        gmm_mask = self.gmm.apply(gray_blur)
        # 降低二值化阈值，兼容不同 OpenCV 版本
        _, gmm_mask = cv2.threshold(gmm_mask, 127, 255, cv2.THRESH_BINARY)

        frame_diff = cv2.absdiff(gray_blur, prev_gray_blur)
        _, diff_mask = cv2.threshold(frame_diff, self.fd_var, 255, cv2.THRESH_BINARY)

        # AND 改 OR，防止 GMM 未及时适应导致漏检
        fg_mask = cv2.bitwise_or(gmm_mask, diff_mask)

        if self.kernel is None:
            self.kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5, 5))

        # 去掉 MORPH_OPEN，只保留闭运算填充空洞
        ...
        # ROI 内只计算 ROI 面积，无 ROI 则计算整帧
        if self.roi_mask is not None and self.roi_mask.shape[:2] == gray.shape[:2]:
            total_pixels = cv2.countNonZero(self.roi_mask)
        else:
...
class GMMVideoDetector:
    def __init__(self, root: tk.Tk):
        ...
        self.save_path = os.path.join(get_pictures_dir(), "变化截图")
        self.log_dir = os.path.join(get_app_data_dir(), "检测日志")
        ...
        self._current_process_size = None
        self._icon_img = None
    ...
    def _init_background_tasks(self):
        ...
        self.init_log_file()
        self.setup_icon()
        self.log_message("GMM程序启动 (v1.3.3 版)")
        self.log_message(f"截图保存路径：{self.save_path}")
        self.log_message(f"日志保存路径：{self.log_dir}")
    ...
    def log_message(self, message: str):
        ...
        with self.log_lock:
            handle = getattr(self, 'log_file_handle', None)
            if handle:
                try:
    ...
    def _setup_styles(self):
        ...
        style.map(
            "Accent.TButton",
            background=[("active", COLORS['primary_active']), ("pressed", "#0050b3")],
            foreground=[("active", "white"), ("pressed", "white")]
        )
        style.configure(
            "Danger.TButton",
        ...
        style.map(
            "Danger.TButton",
            background=[("active", "#d9363e"), ("pressed", "#a8071a")],
            foreground=[("active", "white"), ("pressed", "white")]
        )
        style.configure(
            "TEntry",
    ...
    def create_widgets(self):
        ...
        self.parameter_widgets = [
            ...
            self.log_path_entry, self.log_path_btn
        ]
    ...
    def _create_status_bar(self):
        ...
        self.status_bar_label.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=int(15 * self.dpi_scale))

        version_label = tk.Label(
            status_bar,
            text="https://github.com/geckotao/GMM_detection_video",
            bg=COLORS['bg_card'],
            fg=COLORS['text_main'],
            font=('Microsoft YaHei UI', self.scaled_font_size),
            anchor=tk.E
    ...
    def _create_settings_tab(self, notebook):
        ...
        ttk.Label(settings_scrollable_frame, text="").grid(row=row, column=0, pady=int(15 * self.dpi_scale))

    def _create_file_management_frame(self, parent, row, pady, inner_pad):
        file_frame = ttk.LabelFrame(parent, text="视频文件管理", padding=(inner_pad, int(10 * self.dpi_scale)))
        file_frame.grid(row=row, column=0, sticky="ew", pady=pady, padx=int(8 * self.dpi_scale))
        ...
        self.add_btn.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, int(6 * self.dpi_scale)))
        ...
        self.remove_btn.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, int(4 * self.dpi_scale)))

        self.preview_btn = ttk.Button(action_btn_frame, text="预览", command=self.preview_selected_video)
        self.preview_btn.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(int(4 * self.dpi_scale), 0))

    def _create_roi_frame(self, parent, row, pady, inner_pad):
        roi_frame = ttk.LabelFrame(parent, text="关注区域 (ROI)", padding=(inner_pad, int(10 * self.dpi_scale)))
        roi_frame.grid(row=row, column=0, sticky="ew", pady=pady, padx=int(8 * self.dpi_scale))
        ...
        self.roi_status_label = ttk.Label(roi_frame, text="未选取关注区域", foreground=COLORS['text_sec'])
    ...
    def _create_param_frame(self, parent, row, pady, inner_pad):
        param_frame = ttk.LabelFrame(parent, text="检测参数", padding=(inner_pad, int(10 * self.dpi_scale)))
        param_frame.grid(row=row, column=0, sticky="ew", pady=pady, padx=int(8 * self.dpi_scale))

        # GMM 敏感度
        ttk.Label(
            param_frame,
            text="GMM 敏感度 (varThreshold)",
            font=('Microsoft YaHei UI', self.scaled_font_size, 'bold')
        ).pack(anchor=tk.W, pady=(0, int(3 * self.dpi_scale)))
        ...
        self.gmm_scale = ttk.Scale(
            gmm_f,
            from_=10,
            to=50,
            variable=self.gmm_var,
            command=self.update_gmm_threshold
        )
        self.gmm_scale.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, int(6 * self.dpi_scale)))

        self.gmm_label = ttk.Label(
            gmm_f,
            text=str(DEFAULT_GMM_VAR_THRESHOLD),
        ...
        self.fd_scale = ttk.Scale(
            fd_f,
            from_=5,
            to=60,
            variable=self.fd_var,
            command=self.update_frame_diff_threshold
        )
        self.fd_scale.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, int(6 * self.dpi_scale)))

        self.fd_label = ttk.Label(
            fd_f,
            text=str(DEFAULT_FRAME_DIFF_THRESHOLD),
            width=int(6 * self.dpi_scale),
            font=('Consolas', self.scaled_font_size)
        )
        ...
        self.threshold_scale.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, int(6 * self.dpi_scale)))
        ...
        self.interval_scale.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, int(6 * self.dpi_scale)))

        self.interval_label = ttk.Label(
            intv_f,
            text=f"{self.min_interval:.1f}s",
            width=int(6 * self.dpi_scale),
            font=('Consolas', self.scaled_font_size)
        )
        ...
        self.target_height_scale = ttk.Scale(
            h_f,
            from_=240,
            to=1080,
            variable=self.target_height_var,
            command=self.update_target_height
        )
        self.target_height_scale.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, int(6 * self.dpi_scale)))

        self.target_height_label = ttk.Label(
            h_f,
            text="480",
            width=int(6 * self.dpi_scale),
            font=('Consolas', self.scaled_font_size)
        ...
        self.target_height_entry = ttk.Entry(
            h_f,
    ...
    def _create_save_frame(self, parent, row, pady, inner_pad):
        save_frame = ttk.LabelFrame(parent, text="输出路径设置", padding=(inner_pad, int(10 * self.dpi_scale)))
        save_frame.grid(row=row, column=0, sticky="ew", pady=pady, padx=int(8 * self.dpi_scale))
        ...
        path_f1.pack(fill=tk.X, pady=(0, int(10 * self.dpi_scale)))
        ...
        self.save_path_entry.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, int(4 * self.dpi_scale)))
        self.save_path_btn = ttk.Button(
            path_f1,
            text="更改",
        ...
        path_f2 = ttk.Frame(save_frame)
        path_f2.pack(fill=tk.X, pady=(0, int(6 * self.dpi_scale)))
        self.log_path_var = tk.StringVar(value=self.log_dir)
        self.log_path_entry = ttk.Entry(
            path_f2,
            textvariable=self.log_path_var,
            font=('Microsoft YaHei UI', self.scaled_font_size)
        )
        self.log_path_entry.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, int(4 * self.dpi_scale)))
        self.log_path_btn = ttk.Button(
            path_f2,
            text="更改",
            command=self.change_log_path,
            width=int(6 * self.dpi_scale)
        )
        self.log_path_btn.pack(side=tk.RIGHT)
    ...
    def _create_control_tab(self, notebook):
        ...
        # 状态与进度
        status_progress_frame = ttk.LabelFrame(
            control_frame_nb,
            text="状态与进度",
            padding=(inner_pad, int(10 * self.dpi_scale))
        )
        status_progress_frame.grid(
            row=0,
            column=0,
            sticky="ew",
        ...
        self.total_percent_label = ttk.Label(
            progress_row,
            text="0.0%",
        ...
        for i, s in enumerate(self.speed_levels):
            ...
            btn.grid(row=i // cols, column=i % cols, padx=int(3 * self.dpi_scale), pady=int(3 * self.dpi_scale), sticky="ew")
        ...
        # 控制命令
        control_buttons_frame = ttk.LabelFrame(
            control_frame_nb,
            text="控制命令",
            padding=(inner_pad, int(10 * self.dpi_scale))
        )
        ...
        self.control_btn = ttk.Button(
            btn_frame,
            text="开始处理",
            command=self.on_control_click,
            style="Accent.TButton"
        )
        self.control_btn.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=int(3 * self.dpi_scale))
        ...
        self.stop_btn.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=int(3 * self.dpi_scale))
        self.stop_btn.config(state="disabled")

        # 处理选项
        bg_mode_frame = ttk.LabelFrame(
            control_frame_nb,
            text="处理选项",
            padding=(inner_pad, int(10 * self.dpi_scale))
        )
        ...
        ).pack(anchor=tk.W, padx=int(6 * self.dpi_scale))
    ...
    def _create_help_tab(self, notebook):
        ...
        help_content = """视频画面变化检测工具 v1.3.3 版
...
"""
    ...
    def _create_preview_area(self, padx, pady):
        ...
        display_frame = ttk.LabelFrame(right_frame, text="视频预览", padding=int(10 * self.dpi_scale))
        ...
        preview_container.grid(row=0, column=0, sticky="nsew", padx=int(3 * self.dpi_scale), pady=int(3 * self.dpi_scale))
        ...
        info_bar = tk.Frame(right_frame, height=int(35 * self.dpi_scale), bg=COLORS['bg_card'])
        info_bar.grid(row=1, column=0, sticky="ew", pady=(int(6 * self.dpi_scale), 0))
        info_bar.grid_columnconfigure(0, weight=1)

        self.info_label = tk.Label(
            info_bar,
            text="就绪 - 请添加视频文件并开始处理",
            font=('Microsoft YaHei UI', max(9, int(9 * self.dpi_scale))),
            bg=COLORS['bg_card'],
            fg=COLORS['text_main']
        )
    ...
    def on_preview_resize(self, event=None):
        ...
        if self._last_preview_size is not None and (event.width, event.height) == self._last_preview_size:
            return

        self._last_preview_size = (event.width, event.height)

        frame_to_display = self._last_displayed_frame
        if frame_to_display is not None:
            self.display_frame(frame_to_display)
    ...
    def display_frame(self, frame: np.ndarray):
        ...
        if self.roi_selected and self.roi_points:
            ...
            if orig_w > 0 and orig_h > 0:
                ...
                pts = np.array(
                    [[int(x * sx), int(y * sy)] for x, y in self.roi_points],
                    dtype=np.int32
                )
    ...
    def _show_preview_frame(self, bgr_frame: np.ndarray, video_name: str):
        if bgr_frame is None:
            return

        frame = bgr_frame.copy()
        target_h = self._get_target_height()
        ...
        self.progress_label.config(text=f"预览：{video_name}")
    ...
    def on_control_click(self):
        if self.processing and self.paused:
            self.progress_label.config(text="正在准备继续处理...")
            self.root.update_idletasks()
            ...
            self.progress_label.config(text="继续处理中（如修改检测参数/ROI，将重新初始化当前帧）...")
            ...
            self.control_btn.config(text="继续处理", style="TButton")
    ...
    def _reload_roi_and_params(self):
        ...
        self.log_message("已标记参数/ROI/分辨率重载，将在继续处理时生效")
    ...
    # ==================== 安全取值 ====================
    def _safe_int_var(self, var, default: int) -> int:
        try:
            return int(var.get())
        except Exception:
            return default
    ...
    def _get_target_height(self) -> int:
        v = self._safe_int_var(self.target_height_var, 480)
        if 240 <= v <= 2160:
            return v
        return 480

    def _get_gmm_var(self) -> int:
        return self._safe_int_var(self.gmm_var, DEFAULT_GMM_VAR_THRESHOLD)

    def _get_fd_var(self) -> int:
        return self._safe_int_var(self.fd_var, DEFAULT_FRAME_DIFF_THRESHOLD)
    ...
    # ==================== ROI 相关 ====================
    def _build_roi_mask_original(self) -> Optional[np.ndarray]:
        ...
        try:
            ...
            self.log_message(f"创建原始 ROI 掩码失败：{str(e)}")
            return None

    def _build_processing_roi_mask(self, target_size: Optional[Tuple[int, int]] = None) -> Optional[np.ndarray]:
        mask = self._build_roi_mask_original()
        if mask is None:
            return None
        ...
        if target_size is not None:
            try:
                ...
                self.log_message(f"缩放 ROI 掩码失败：{str(e)}")
                return None
    ...
    def create_roi_mask(self, target_size: Optional[Tuple[int, int]] = None):
        ...
        try:
            ...
            if target_size is not None:
                self.processing_roi_mask = cv2.resize(mask, target_size, interpolation=cv2.INTER_NEAREST)
            else:
                self.processing_roi_mask = mask

            area_ratio = cv2.countNonZero(mask) / mask.size * 100
            self.roi_status_label.config(
                text=f"已选取 ROI 区域 ({area_ratio:.1f}% 画面)",
    ...
    def _update_preview_from_current_capture(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                self.log_message(f"恢复预览读取位置失败：{e}")
            ...
            self._show_preview_frame(frame, name)
            return True

        except Exception as e:
            self.log_message(f"从当前视频读取预览帧失败：{e}")
            return False

    def _ensure_preview_frame_for_roi(self) -> bool:
        if self.processing and self.paused:
            return self._update_preview_from_current_capture()

        selected = self.video_listbox.curselection()
        if not selected and self.video_paths:
    ...
    def select_roi(self):
        ...
        if not self._ensure_preview_frame_for_roi():
        ...
        def draw_roi():
            ...
            for i, (x, y) in enumerate(scaled):
                ...
                if i > 0:
                    lines.append(canvas.create_line(scaled[i - 1][0], scaled[i - 1][1], x, y, fill="#52c41a", width=3))
        ...
        def update_image():
            new_size = (
                int(self.roi_original_size[0] * self.roi_scale_factor),
                int(self.roi_original_size[1] * self.roi_scale_factor)
            )

            resized_img = self.roi_image.resize(new_size, PIL_LANCZOS)
        ...
        def fit_to_window():
            win_w, win_h = canvas.winfo_width(), canvas.winfo_height()
    ...
    def _reinitialize_processor(self, old_processor: VideoProcessor, old_target_h: int) -> Tuple[VideoProcessor, int]:

        if not (self.cap and self.cap.isOpened()):
            return old_processor, old_target_h
        ...
        try:
            ...
            frame_height, frame_width = proc_frame.shape[:2]
            if target_h > 0 and frame_height > target_h:
                scale = target_h / frame_height
                new_w = int(frame_width * scale)
                proc_frame = cv2.resize(proc_frame, (new_w, target_h), interpolation=cv2.INTER_AREA)
                frame_height, frame_width = proc_frame.shape[:2]

            if self.roi_selected:
                roi_mask = self._build_processing_roi_mask((frame_width, frame_height))
            else:
                roi_mask = None

            self.processing_roi_mask = roi_mask
            self._current_process_size = (frame_width, frame_height)

            processor = VideoProcessor(
                gmm_var=self._get_gmm_var(),
                fd_var=self._get_fd_var(),
                roi_mask=roi_mask
            )

            _, gray = processor.preprocess_frame(proc_frame, target_h)
            processor.detect_change(gray)
            ...
            try:
                ...
                self.log_message(f"重初始化后恢复视频位置失败：{e}")

            self.log_message("已重新初始化检测器（参数/ROI/分辨率变更）")
            return processor, target_h

        except Exception as e:
            self.log_message(f"重新初始化检测器失败：{e}")
            return old_processor, old_target_h
    ...
    def process_videos(self):
        try:
            ...
            while current_index < len(self.video_paths):
                ...
                if self.roi_selected:
                    self.processing_roi_mask = self._build_processing_roi_mask((frame_width, frame_height))
                else:
                    self.processing_roi_mask = None
                ...
                _, gray_first = processor.preprocess_frame(first_frame, target_h)
                processor.detect_change(gray_first)
                ...
                try:
                    ...
                    self.log_message(f"重置视频位置失败：{e}")

                frame_cursor = 0
                last_saved_time = 0.0

                while not self._stop_event.is_set():
                    if total_frames > 0 and frame_cursor >= total_frames:
                        break

                    # 暂停等待：不会 CPU 空转
                    if self.paused or not self._resume_event.is_set():
                        self._resume_event.wait(timeout=0.1)
                        continue

                    # 继续处理时，如有参数/ROI/分辨率变更，重新初始化检测器
                    if self._reload_requested:
                        processor, target_h = self._reinitialize_processor(processor, target_h)
                        self._reload_requested = False

                    read_failed = False

                    # 倍速跳帧：使用 grab，不额外解码，更快
                    ...
                    if self._stop_event.is_set() or read_failed:
                        break
                    ...
                    actual_frame_index = frame_cursor
                    frame_cursor += 1

                    original_frame, gray = processor.preprocess_frame(frame, target_h)
                    if gray is None:
                        continue
                    ...
                    if change_ratio >= self.change_threshold:
                        ...
                        if debug_now - self._last_debug_log_time > 1.0:
                            self._last_debug_log_time = debug_now
                            self.log_message(
                                f"变化率达标：{change_ratio * 100:.2f}%，"
                                f"阈值：{self.change_threshold * 100:.2f}%，"
                                f"valid_change={valid_change}，"
                                f"fg_pixels={cv2.countNonZero(fg_mask)}"
                            )
                    # 画面变化率达到阈值即截图
                    if (
                        change_ratio >= self.change_threshold
                        and (now - last_saved_time) > self.min_interval
                    ):
                        last_saved_time = now
                        video_basename = os.path.splitext(os.path.basename(video_path))[0]
                        self.save_screenshot(original_frame.copy(), video_basename, actual_frame_index)
                    ...
                    if not self.background_mode_var.get():
                        ...
                        if now_time - self._last_preview_update_time >= PREVIEW_UPDATE_INTERVAL:
                            ...
                            if fg_mask.shape[:2] != (h, w):
                                fg_mask_draw = cv2.resize(fg_mask, (w, h), interpolation=cv2.INTER_NEAREST)
                            else:
                                fg_mask_draw = fg_mask

                            color_mask = np.zeros_like(original_frame)
                            color_mask[:, :, 2] = fg_mask_draw
                            ...
                            if self.roi_selected and self.roi_points and self.preview_frame is not None:
                                ...
                                if orig_w > 0 and orig_h > 0:
                                    pts = np.array(
                                        [[int(x * w / orig_w), int(y * h / orig_h)] for x, y in self.roi_points],
                                        dtype=np.int32
                                    )
                    ...
                    # 进度
                    if total_frames > 0:
                        percent_in_video = min(100.0, frame_cursor / total_frames * 100)
                        overall_progress = ((current_index + frame_cursor / total_frames) / len(self.video_paths)) * 100
                        frame_text = f"[{frame_cursor}/{total_frames}]"
                    else:
                        percent_in_video = 0.0
                        overall_progress = (current_index / len(self.video_paths)) * 100
                        frame_text = f"[{frame_cursor}/?]"
                    ...
                    self.safe_ui_call(
                        ...
                        text=f"以{self.current_speed}x 处理第{current_index + 1}个 {frame_text}|共{len(self.video_paths)}个视频"
                    )
            ...
            if not self._stop_event.is_set():
                ...
                self.safe_ui_call(self._stop_cleanup, "complete")
            ...
            self.safe_ui_call(messagebox.showerror, "错误", error_msg)
            self.safe_ui_call(self._stop_cleanup, "stopped")
        finally:
            self._release_capture()
    ...
    def toggle_background_mode(self):
        ...
        self.info_label.config(
            text=f"后台模式已{mode} - 预览{'禁用' if self.background_mode_var.get() else '启用'}"
        )
    ...
    def preview_selected_video(self):
        ...
        if self.processing and not self.paused:
            messagebox.showinfo("提示", "请先暂停或停止处理，再预览视频")
            return

        if self.processing and self.paused:
            if self._update_preview_from_current_capture():
                return
            messagebox.showwarning("警告", "无法从当前处理视频获取预览帧")
            return

        video_path = self.video_paths[selected[0]]

        cap, err = self.safe_video_capture(video_path)
        if err:
        ...
        if ret and frame is not None:
            self.preview_frame = frame.copy()
            self._show_preview_frame(frame, os.path.basename(video_path))
        else:
    ...
    def change_log_path(self):
        new_path = filedialog.askdirectory(title="选择日志保存路径")
        if new_path:
            try:
                ...
                self.log_path_var.set(new_path)
                self.init_log_file()
                self.log_message(f"日志保存路径更改至：{new_path}")
                self.info_label.config(text=f"日志路径：{os.path.basename(new_path)}")
            except Exception as e:
                messagebox.showwarning("路径警告", f"所选路径不可写:\n{str(e)}")
                self.log_message(f"日志路径不可写：{str(e)}")
    ...
    def _stop_cleanup(self, reason: str = "stopped"):
        self.processing = False
        self.paused = False
        ...
        self.control_btn.config(text="开始处理", style="Accent.TButton", state="normal")
        self.stop_btn.config(state="disabled")
        self.control_btn.focus_set()

        if reason == "complete":
            self.status_var.set("处理完成")
            self.progress_label.config(text="所有视频处理完毕")
        ...
        self.root.after(50, lambda: self.control_btn.config(text="开始处理", style="Accent.TButton"))
    ...
    def stop_processing(self):
        ...
        self.root.after(100, lambda: self._stop_cleanup("stopped"))

    def save_screenshot(self, frame: np.ndarray, video_basename: str, frame_num: int) -> Optional[str]:

        try:
            safe_name = "".join(
                c for c in video_basename if c.isalnum() or c in (' ', '-', '_')
            ).rstrip()
            ...
            self.safe_ui_call(self.info_label.config, text=f"已保存截图：{filename}")
            return full_path

        except Exception as e:
            self.log_message(f"保存截图失败：{str(e)}")
            self.safe_ui_call(messagebox.showerror, "保存错误", f"保存截图失败:\n{str(e)}")
